...
public abstract class ThrottledApiClient {
    private static final Logger log = LogManager.getLogger(ThrottledApiClient.class);
    ...
    ThrottledApiClient(Vertx vertx) {
        this.vertx = vertx;
    ...
    public abstract String getRateLimitHeader();
        ...
        return timerId -> {
            ...
            if (context != null) {
                ...
                request.send(response -> {
                    log.debug("executor() response for : {}", queryTerm);
    ...
    private void checkAndUpdateRateLimit(HttpResponse<Buffer> response) {
        Optional.ofNullable(response.getHeader(getRateLimitHeader()))
            .map(Long::parseLong)
